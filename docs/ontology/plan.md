...
| S20 | Client cache scope narrowing — kill flat `session.json` shared file + close umask gap on direct-writer | **Open 2026-04-25** — plan doc shipped at `docs/ontology/s20-cache-scope-narrowing.md` (reviewed by `dialectic-knowledge-architect` + `feature-dev:code-reviewer` council; one-pass honesty re-review). Addresses cache *placement* (orthogonal to S2's *semantic* auto-resume retire and to S1's *token-format* cuts). Two surfaces: (i) plugin helper `unitares-governance-plugin/scripts/session_cache.py:_cache_path` falls back to flat `session.json` when callers omit `--slot`, becoming workspace's "current owner" file readable by every same-UID process; (ii) `unitares/scripts/client/onboard_helper.py:98-101, 234-245` writes the cache *directly* via `path.write_text` (umask-default 0644) and includes `continuity_token` in the payload — bypasses the plugin helper entirely. Honestly labeled **convention-level, advisory** — bypassable by any caller writing JSON to the path; the earned defense is S1-A′ + S19. S20's job is to stop the system from *teaching* the shared-cache pattern, not to prevent a determined caller from re-creating it. | S20.0 (empirical Claude Code session-ID stability check) blocks S20.2; helper enforcement (S20.1) is independent. | **S20.0:** Verify Claude Code session ID stable across `/clear` + compaction + restart; if not, §3b collapses to scan-newest fallback only. **S20.1:** `cmd_set` rejects slotless writes (`--allow-shared` opt-in for substrate-earned single-tenant); `cmd_set` rejects v2 payloads carrying `continuity_token`; new `cmd_list` for slot inventory. **S20.2:** `hooks/session-start` rewired to platform-slot lookup → `cmd_list` fallback (replaces flat-file-only read at line 102). **S20.3:** `onboard_helper.py` parity — converge on helper (C1, preferred) or mirror contract locally with mode 0600 + token rejection (C2). **S20.4:** Codex equivalents. **S20.5:** Operator-runbook migration note. **S20.6:** Tests for helper rejection, bootstrap reads, flat-as-legacy, direct-writer mode. |
| S11-a | Skill text drift from S11 contract — `commands/governance-start.md` still teaches `continuity_token` cache pattern | **Open 2026-04-25** — plan doc shipped at `docs/ontology/s11a-skill-text-drift.md`. **S11 regression**, not a new concern. The `unitares-governance-plugin/commands/governance-start.md` (also surfaced to Claude via Skill tool path — see channel-bleed memory note) still tells agents to (a) prefer/include `continuity_token`, (b) run `set session --merge --stamp` without `--slot`, (c) persist `continuity_token`/`continuity_token_supported` in cache. Agents following this guidance write v1-pattern flat caches the post-identity hook then overwrites with v2 — net effect depends on read-order. Single-file text edit. | None. S20 and S11-a ship independently; if S20.1 lands first, helper rejection forces the skill into compliance. | (a) `commands/governance-start.md` rewritten: drop `continuity_token` from prefer/include lists; require `--slot=<harness-session-id>` on `set session` (or remove cache-write step if S20.1 already enforces); drop `continuity_token`/`continuity_token_supported` from persisted-fields list; declarative-lineage framing (`parent_agent_id`, not token resume). (b) Lint/grep test asserting plugin command files contain no `continuity_token` outside deprecation comments. (c) Note in S11-a §5 that pre-existing flat token-bearing caches on disk persist until S20.3 ships + operator cleanup. |
...
S20 ── (nothing) ── actionable now; cache placement orthogonal to S1/S2; S20.0 empirical-check gates S20.2
