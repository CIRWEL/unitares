...
|---|---|---|---|
| R1 | Behavioral-continuity verification as primary identity primitive | None (design from scratch) | **Design doc landed 2026-04-24** at `docs/ontology/r1-verify-lineage-claim.md` — five-channel similarity model (trajectory shape, homeostatic set-point, calibration, decisions, complexity), `LineageVerdict` signature, threshold bands (0.75 / 0.40), implementation sketch against existing `trajectory_identity.py` primitives, synthetic-data test-fixture plan. Open for operator review; no implementation committed. |
