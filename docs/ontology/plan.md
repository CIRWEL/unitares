...
| S15 | Server-side skills surface — promote `skills/` from per-client artifact to server-authored MCP tool | **Plan doc shipped 2026-04-25** at `docs/ontology/s15-server-side-skills.md` (reviewed by `dialectic-knowledge-architect` + `feature-dev:code-reviewer` council). Surfaces structural drift (5 of 6 shared skills differ between `unitares/skills/` and `unitares-governance-plugin/skills/`; plugin-only `unitares-dashboard`) and cross-client governance-literacy gap (motivating example: 2026-04-25 Hermes session lacking a UNITARES skill bundle proposed to auto-inject `continuity_token` between MCP calls — exactly the silent-resurrection vector S1 is narrowing). Recommendation **A** (server-side `skills` MCP tool) with canonical content in `unitares/skills/`, plugin bundle becomes generated mirror, per-client adapters (S15-c+) consume canonical content. Identity-blindness invariant locked at §4.5 (cache must not be keyed by `agent_uuid`); authority hierarchy locked at §6 (tool descriptions authoritative for tool-call invariants; skills supplement, never contradict); authz-non-encoding guard at §8.4. Honest scope: S15-a alone closes the **structural** drift problem on the server side; the **Hermes incident** is closed when S15-e (Hermes adapter) ships, which is not unitares scope. Cheaper complementary cure identified: §6 recommends a small `src/tool_descriptions.json` audit embedding v2-ontology invariants (no auto-inject token; lineage via `parent_agent_id`) — closes the immediate Hermes-class failure across every MCP client with zero adapter work, ships independently. | None. Scope clear; sequencing operator-decided per §11. | S15-a: server `skills` MCP tool at `src/mcp_handlers/introspection/skills.py` + Pydantic schema + `__init__.py` registration + `rate_limit_step.py` `read_only_tools` entry, shipped to `unitares` master. S15-b: skill content consolidation (resolve the 5/6 divergence + canonical decision on `unitares-dashboard`); recommended **paired with or before** S15-a per §11.9 to avoid shipping the new tool pointing at known-divergent source-of-truth. S15-c: Claude Code adapter (plugin-side, fetch-from-server with bundled-mirror offline fallback). S15-d: plugin bundle becomes generated mirror via `scripts/dev/sync-plugin-skills.sh` + CI parity gate. S15-e (optional, Hermes-side): Hermes skill adapter — closes the originating incident. S15-f (optional, no-op this side): claude.ai surfaces skills via existing Cloudflare-tunneled MCP. Optional companion PR per §6: `tool_descriptions.json` v2-ontology audit (highest-leverage governance-literacy fix, ships independently of S15-a). |
