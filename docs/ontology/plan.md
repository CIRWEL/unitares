...
**Critical path now (2026-05-02 refresh):** S21 is closed end-to-end (S21-a 2026-04-28, S21-b via #231 + #245), S1-a/S1-b are shipped, S8a Phase 2 is shipped (#252), and **R2 design v2 is shipped 2026-05-02** (operator decisions accepted same day on cross-role-reject + v1.1-defer-cite-and-extend + clawback-over-fork). Remaining near-term work: (a) schedule/run S8b backfill with `s8a_phase2_backfill.py`, (b) ~~open R2 design~~ → now: open R1 implementation row once S8c data window has aged through ≥2026-05-02 and S8a Phase 2 corpus is legible (R2 implementation row sequences after R1's), (c) promote R6/S22 response-shape decisions for harness/episode provenance, and (d) keep the `gh pr list` preflight discipline from the 2026-04-29 migration-drift collision before any new identity ontology or migration-slot work.
