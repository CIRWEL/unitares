...
RESOLVED (shipped or accepted)
  ...
  S21-b    ⚠️ 2026-04-29  item 3 (auth consistency) shipped as #231 + 8 adjacent hardening commits (substrate-token reject ordering, KG identity preservation, diagnostic session binding, trust-tier drift, cache-hydration suppression, UDS resident anchor uuid-only, resident check-in timestamp alignment); audit pass 2026-04-29 evening — item 7 ✅ (`identity_resolution_outcome` field shipped at `resolution.py:_created_identity_outcome` returning `minted_force_new` / `minted_after_resume_miss` / `minted_fresh` / `resumed`; PATH 1/2 paths emit "resumed", PATH 3 mints classify by force_new + spawn_reason); item 8 ✅ shipped under different event name — `log_session_resolve_miss_observed` fires from `_audit_session_resolve_miss` at `resolution.py:679` on PATH 2 fall-through MISS (the rejected-explicit-csi case); the S13 §(d) `concurrent_session_binding_observed` event also exists at `session.py:693` for pin-match observation; item 5 ❌ NOT shipped — both call sites still present (`identity_step.py:414` + `:454` and `handlers.py:480/797/1295/1489/1494/1534`), plus a third location at `http_api.py:312`; item 6 ⚠️ partial — `agent_auth.py:require_registered_agent` still reads `agent_metadata` exclusively; what shipped (lines 332-357) is an S21-b §2 status-gate mitigation that fails-closed on `meta.status` not in {active, paused, waiting_input}, with a comment acknowledging the in-memory dict can be stale relative to `core.identities` — a mitigation, not the lookup-against-truth-source the plan-row specified. Items 5 + 6 carry forward to a focused follow-up.
...
OPEN — actionable
  ...
  S21-b    items 5 + 6 follow-up (audit 2026-04-29 closed 7 + 8): item 5 — consolidate dual `resolve_session_identity` sites at `identity_step.py:414` + `handlers.py:797` (cleanest approach: pass middleware-resolved identity through `arguments` and have `handle_onboard_v2` consume it instead of re-resolving); item 6 — promote `require_registered_agent` (`agent_auth.py:256`) from in-memory-status-gate to `core.identities` lookup so stale-active dict entries can't pass auth (status-gate is a mitigation; sync gap with truth source remains)
