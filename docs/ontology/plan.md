...
OPEN — actionable
  S1       S1-a partly shipped (TTL/version fields in master); remaining: identity()/bind_session/HTTP wiring + clock-skew + 3 regression tests
...
**Critical path now (post-2026-04-27 sweep):** the four hard blockers from the 2026-04-25 council pass — S13, S8c, S15-a, S19-PR1 — have all shipped. Remaining near-term sequencing: (a) finish S1-a wiring (identity()/bind_session/HTTP paths + clock-skew + tests), (b) S8a Phase 2 ratification once ≥1 week data accumulates (~2026-04-30), (c) S19-PR2 (UDS listener) before any other resident attestation work, (d) R1 implementation row opens after S8c data ages + Phase 2 — earliest mid-May.
