...
|---|---|---|---|
| R1 | Behavioral-continuity verification as primary identity primitive | None (design from scratch) | **Design doc v3 landed 2026-04-24** at `docs/ontology/r1-verify-lineage-claim.md` after two council-review passes (v1→v2, v2→v3). Final form: **single-channel** `score_trajectory_continuity` primitive — per-dimension DTW on EISV series reconstructed server-side from `core.agent_state` rows. v2's C1/C2 channels were both gated on agent-uploaded `TrajectorySignature.attractor`; v3 bypasses via a new thin query helper (`reconstruct_eisv_series`), restoring implementability on the standard `process_agent_update` path. C2/C3/C4/C5 all deferred with named prerequisites (fit machinery, per-agent calibration storage, persistent decision log, complexity samples). No weighting questions remain (one channel). Thresholds seeded from synthetic fixtures but explicitly require shadow-mode calibration before any caller treats them as enforcement-worthy. Caller-policy responsibility surfaced: each call-site must pick `blocks` or `marks` for `inconclusive`. R1's near-term value stated directly as diagnostic (shadow-mode telemetry), not functional — becomes load-bearing only when R2 consumes it. **Design accepted by operator 2026-04-25** with three resolutions to the doc's open questions: (1) caller-policy list (onboard=marks; promotion=blocks; orphan=blocks) accepted as-is; (2) shadow-mode cutoff bumped to **≥100 pairs OR ≥4 weeks, whichever later** (was ≥50/≥2 weeks; corpus-maturity caution from Schmidt n=15 generalizes); (3) `reconstruct_eisv_series` helper ships in the R1 implementation row, not split out. **Implementation in flight 2026-05-03:** PRs 1–4a shipped (#306 foundation + migration 031 + `reconstruct_eisv_series` + epoch backport; #309 core primitive `score_trajectory_continuity` + audit fail-loud + redacted public payload shape; #314 calibration_state singleton + class_tag stamping + verdict degradation + provisional helpers via migrations 032/033; #320 trust-tier provisional gate, v3.3-D consumer 1 of 4). **§4.1 onboard wiring with marks policy shipped 2026-05-03 ([#321](https://github.com/CIRWEL/unitares/pull/321))** — `_score_lineage_continuity_bg` fires `score_trajectory_continuity` in a tracked background task on both fresh-identity-with-lineage onboard paths; `mark_lineage_provisional` called on `verdict='inconclusive'`. The 4-week shadow window per the operator's 2026-04-25 decision (≥100 pairs OR ≥4 weeks) begins on this merge. **Operator-facing consequence (named in #321 review):** every fresh agent declaring lineage at onboard is marked provisional and pinned to tier=1 via the v3.3-D trust-tier consumer; lineage-claiming agents stay tier=1 indefinitely until promotion (`confirm_lineage` caller) is wired. **§4.3 public KG emission in flight at [#324](https://github.com/CIRWEL/unitares/pull/324)** — closes PR 2's deferred AGE write; redirected to PG-FTS (`knowledge.discoveries`) per v3.3-I correction; v3.2-D dedupe-by-pair via deterministic `r1_score:{uuid5(...)}` node id + existing ON CONFLICT (id) DO UPDATE. **Still open:** §4.2 remaining v3.3-D consumers (KG provenance S7, R3 baselines, dashboard); promotion + orphan call-site wiring (`confirm_lineage`); 30-day TTL archival of public KG nodes (v3.2-D second clause); §4.4 perf — thread `prefetched_provisional` through 3 `resolve_trust_tier` callers when `IdentityRecord` gains the field. Handoff snapshot at `docs/handoffs/2026-05-03-r1-implementation-handoff.md`. |
| R2 | Honest memory integration | R1 (verification underpins integration checks) | **Design doc v2 landed 2026-05-02** at `docs/ontology/r2-honest-memory-integration.md` after parallel three-agent council pass (`dialectic-knowledge-architect` + `feature-dev:code-reviewer` + `live-verifier`). v1→v2 driven by convergent forcing items: (1) recommendation A retroactive trust-tier crediting violated the asymmetry claim at the *interpretive* layer — switched to **forward-only chain crediting** in v2 (chain counter starts at 0 at promotion; no retroactive credit of parent's accrued history); (2) v1 conflated R1's trajectory-similarity score with axiom #12's "memory operative on behavior" — v2 explicitly frames R1 as **necessary-but-not-sufficient** with R5 as the load-bearing follow-on for replay discrimination; (3) multi-generation chains specified per-link only (no transitive trajectory windowing or transitive trust-tier inheritance); (4) R1 confirmed entirely unimplemented at runtime — v2 adds an explicit "Implementation status of dependencies" table at top; (5) `provisional_lineage` column confirmed nonexistent — v2 commits to column-on-`core.identities` storage (7 new columns) rather than v1's "reuses R1 v3.2-B" false premise; (6) check-in-triggered eval would deadlock under anyio — v2 specifies `create_tracked_task` deferred dispatch pattern. v2 also adds cross-role pre-check at declaration (rejects with `lineage_cross_role_rejected` instead of silently demoting downstream), completes the FSM (cross-role rejection branch + confirmed→demoted clawback), pairs onboard-response addition with identity-response addition, and renames `lineage_archive_grace_expired → lineage_grace_expired` for naming consistency. **Operator decisions 2026-05-02** on three open questions: (a) cross-role lineage = reject at declaration (clean audit semantics over flag-and-allow signal); (b) KG cite-and-extend = defer to v1.1 (maintain R1's single-channel discipline; revisit after Phase 1 trajectory-only telemetry); (c) confirmed→demoted = clawback in v1 (escalate to fork in v1.1 if oscillation surfaces). **Implementation row OPENED 2026-05-04** at `docs/handoffs/2026-05-04-r2-implementation-plan.md`. Both prerequisites resolved: S8c shipped via #155 (2026-04-25), S8a Phase 2 shipped via #252 (2026-05-01), R1 implementation row complete in master (4 PRs + PR 4b doc-deferral, 2026-05-04). Plan sequences as 5 PRs for Phase 1 (storage + audit + FSM + cross-role + sweeper + check-in trigger, no downstream wiring). Reconciliation surfaced: R1 PR #306 (migration 031) already shipped 4 of the 7 columns the design doc named — migration 035 adds only the genuinely new ones (`lineage_declared_at`, `lineage_demoted_at`, `lineage_archived_at`, `lineage_last_eval_at`, `chain_obs_count`); `confirmed_at` from R1 satisfies what the design doc called `lineage_promoted_at`. Phase 2 (downstream consumers: trust-tier S6, KG provenance S7, R3 baselines, dashboard) deferred to separate plan-row gated on ≥4 weeks Phase 1 telemetry per design doc §"Shadow-mode calibration path" (≥50 promoted pairs + ≥10 demoted pairs + ≥1 rejected-cross-role event observed). |
...
| R6 | Harness-substrate plurality — model identity across variable harness/model/transport/tool/memory layers | None; informs S7/S22 and gives R2 better vocabulary | Draft plan landed 2026-04-29 at `docs/ontology/harness-substrate-plurality.md`. **Episode-fork response-shape decision v2 landed 2026-05-02** at `docs/ontology/r6-episode-fork-response-shape.md` after parallel three-agent council pass (architect + code-reviewer + live-verifier; all three returned "withhold pending v2" on v1's 5-value enum + spawn_reason allowlist + onboard-side scope). v2 narrowed scope to `process_agent_update` enrichment surface only (onboard-side `build_fork_context` rebuild gated on prerequisite call-site bug fix at `handlers.py:1946` — flagged as separate row). Promotes 2 fields to S22: `episode_fork_kind` (3-value enum: `none / sibling_locus / identity_lineage`) + `identity_lineage_fork` (boolean). Classifier uses structural rule (`has_child_uuid AND parent_uuid → identity_lineage`) with sync-race fallback (handlers.py:1690-1698 silent-failure path); handles all 6 runtime spawn_reason values without enumerating them. Lumen substrate-earned restart documented as intentional collapse to `sibling_locus`. Honest-message language passes through R2 v2's axiom-#12-aware filter. **Operator decisions 2026-05-02** on three open questions: (a) build_fork_context call-site fix = separate prerequisite row, not bundled with R6 implementation; (b) sync-race detection = warning log only in v1, promote to audit event in v1.1 if Phase 1 shows non-trivial frequency; (c) `substrate_restart` enum value = defer; intentional collapse stands. **R6 implementation row** opens once the call-site prereq lands (or scope narrowed to enrichment-only at impl time). H1/H3 dogfood + broader candidate envelope still gated on more controlled fresh-process/profile runs. |
...
| S22 | Harness context provenance — preserve harness/model/transport/tool-surface context on governance writes and KG provenance | Schema / response-shape enhancement | R6, S7 | Resolved when `process_agent_update` can record optional harness/model/transport/tool-surface metadata; KG writes expose that metadata in provenance; identity responses explicitly distinguish UUID, label, harness, and assurance; and Hermes, Claude Code, and Codex CLI each have one comparable recorded task entry. Draft framing at `docs/ontology/harness-substrate-plurality.md`. **Partial promotion 2026-05-02** via R6 v2 (`docs/ontology/r6-episode-fork-response-shape.md`): two fields (`episode_fork_kind` enum + `identity_lineage_fork` boolean) promoted to `process_agent_update` `thread_context` enrichment; remaining fields (harness/model/transport/tool-surface metadata, full candidate envelope) still gated on dogfood evidence per `harness-substrate-plurality.md` §"Design risks." S22 closure requires the broader envelope work; R6 v2 is the first concrete promotion under it. |

## Status board (snapshot 2026-05-01)
...
OPEN — actionable
  R2       implementation row OPENED 2026-05-04 — see docs/handoffs/2026-05-04-r2-implementation-plan.md (5-PR Phase 1 sequence: migration 035 + storage helpers, FSM core + audit events, cross-role pre-check + onboard wiring, sweeper task, check-in trigger; Phase 2 downstream wiring deferred to separate row gated on ≥4 weeks Phase 1 telemetry per design doc §"Shadow-mode calibration path")
...
**Critical path now (2026-05-02 refresh):** S21 is closed end-to-end (S21-a 2026-04-28, S21-b via #231 + #245), S1-a/S1-b are shipped, S8a Phase 2 is shipped (#252), **R2 design v2 is shipped 2026-05-02** (operator decisions accepted same day), and **R6 episode-fork response-shape decision v2 is shipped 2026-05-02** (partial S22 promotion: `episode_fork_kind` + `identity_lineage_fork` for `process_agent_update` enrichment; onboard-side rebuild gated on a separate prerequisite row). Remaining near-term work: (a) schedule/run S8b backfill with `s8a_phase2_backfill.py`, (b) open R1 implementation row once S8c data window has aged through ≥2026-05-02 and S8a Phase 2 corpus is legible (R2 implementation row sequences after R1's), (c) ~~promote R6/S22 response-shape decisions~~ → R6 partial promotion shipped; remaining S22 envelope still gated on dogfood evidence; (d) **new** — open the `build_fork_context` call-site fix prerequisite row (handlers.py:1946 silent TypeError, blocks onboard-side rich `thread_context` for all `force_new=true` agents), (e) keep the `gh pr list` preflight discipline from the 2026-04-29 migration-drift collision before any new identity ontology or migration-slot work.
