...
|---|---|---|---|
| R1 | Behavioral-continuity verification as primary identity primitive | None (design from scratch) | **Design doc v2 landed 2026-04-24** at `docs/ontology/r1-verify-lineage-claim.md` after dialectic + code-review revision pass. Renamed `verify_lineage_claim` → `score_behavioral_continuity`; dropped security-gate framing. Two-channel minimum spec (C1 trajectory DTW, C2 homeostatic composite); C3 per-agent calibration / C4 decision log / C5 complexity distribution **deferred** behind the same storage-infrastructure gate that paces v7 (`calibration_checker` is global-aggregate; `AgentBaseline.recent_decisions` is 20-entry in-memory; `baseline_complexity` is single EMA scalar). Independence claim retracted — no weights beyond equal ship until pairwise-MI is measured in shadow mode. Narrower R2/R5 claim: R1 provides similarity gate, does not unblock R2/Q1/S9. Open for operator review; no implementation committed. |
