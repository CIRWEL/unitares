...
**Critical path now (2026-05-01 refresh):** S21 is closed end-to-end (S21-a 2026-04-28, S21-b via #231 + #245), S1-a/S1-b are shipped, and S8a Phase 2 is shipped (#252). The remaining near-term work is no longer identity-default plumbing; it is (a) schedule/run S8b backfill with `s8a_phase2_backfill.py`, (b) open R2 honest-memory-integration design while R1 waits for post-S8c data age and v3.2 council, (c) promote R6/S22 response-shape decisions for harness/episode provenance, and (d) keep the `gh pr list` preflight discipline from the 2026-04-29 migration-drift collision before any new identity ontology or migration-slot work. R1 implementation should not open until the S8c data window has aged through 2026-05-02 and the S8a Phase 2 corpus/backfill state is legible.
