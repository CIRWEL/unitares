...
## Status board (snapshot 2026-04-27)
...
RESOLVED (shipped or accepted)
  ...
  S8a      ✅ 2026-04-23  Phase 1 shipped (PR #121); Phase 2 awaits ≥1 week data (~2026-04-30)
...
OPEN — actionable
  S21      Surfaced 2026-04-27 dogfood; hard blocker for S1-a remaining work. S21-a (stop-the-bleed) + S21-b (architectural cleanup); plan doc shipped
  S1       S1-a partly shipped (TTL/version fields in master); remaining wiring BLOCKED on S21-a — without it the TTL-narrowed token would silently invalidate via mid-process re-mint
  ...
  S19      PR1 merged (#164); PR2 (UDS listener + peer_attestation.py) not yet opened
...
BLOCKED — by upstream work
  ...
  R2       blocked on R1
  R5       blocked on R2
  S2, S3   blocked on S1-a → S1-c sequencing
  S9       blocked on R1 (also: bind_session TTL silently coupled to S1-a's 1h TTL)
  S10      blocked on S7
...
**Critical path now (post-2026-04-27 sweep):** the four hard blockers from the 2026-04-25 council pass — S13, S8c, S15-a, S19-PR1 — have all shipped. **NEW BLOCKER 2026-04-27:** S21 surfaced via dogfood — explicit `client_session_id` is silently overwritten in Redis by every PATH 3 ghost-mint, producing chronic 95.1%/30d fleet-wide ghost-fork rate. **S21-a must ship before any further S1-a wiring lands** — without it, S1-a's TTL-narrowed `continuity_token` would be silently invalidated by mid-process re-mints and the deprecation telemetry would measure phantom-rebinds, not genuine token use. Remaining near-term sequencing: (a) **S21-a (stop-the-bleed) — top priority**, (b) finish S1-a wiring after S21-a (identity()/bind_session/HTTP paths + clock-skew + tests), (c) S21-b (architectural cleanup) follows S21-a as separate PR, (d) S8a Phase 2 ratification once ≥1 week data accumulates (~2026-04-30), (e) S19-PR2 (UDS listener) before any other resident attestation work, (f) R1 implementation row opens after S8c data ages + Phase 2 — earliest mid-May.
