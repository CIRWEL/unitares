...
- [x] **§7.2 — invalid scheme rejected at storage layer** — INSERT with `surface_id='not_a_scheme:foo'` raises CHECK violation. Test name: `test_invalid_uri_scheme_rejected_at_storage` (`tests/test_lease_plane_storage_constraints.py`).
- [x] **§7.2 — Pydantic field_validator rejects invalid scheme** — `AcquireRequest(surface_id='potato:foo', ...)` raises ValidationError. Test name: `test_acquire_request_rejects_invalid_scheme` (`tests/test_lease_plane_canonicalize.py`).
- [x] **§7.2.3 — surface_kind drift impossible** — INSERT with `surface_id='file:///x.py'` produces `surface_kind='file'` automatically (generated column); caller cannot supply a conflicting value. Test names: `test_surface_kind_derived_from_scheme` (`tests/test_lease_plane_storage_constraints.py`), `test_acquire_request_has_no_surface_kind_field` (`tests/test_lease_plane_canonicalize.py`).
...
- [x] **§7.3.3 — `acquire_with_retry()` honors backoff** — jittered exponential, floor 100ms, ceiling 5s. Test name: `test_acquire_with_retry_jittered_backoff_within_bounds` (`tests/test_lease_plane_retry_and_transport.py`).
- [x] **§7.3.5 — HTTP 409 on `held_by_other`; 200 + ok:false otherwise** — Elixir router behavior. Test names (Elixir-side): `test http_router returns 409 on held_by_other` (`elixir/lease_plane/test/http_router_test.exs:230`), `test http_router returns 200 on permission_denied` (`elixir/lease_plane/test/http_router_test.exs:215`). Canonical 200-trigger is the §4.4 role-holder rejection at `http_router.ex:310`; §7.10 forced-release-on-/release rejection at `http_router.ex:356` is a second path. `:not_found` → 404 on /release and /handoff_accept is permitted by §7.3.5 text ("HTTP 4xx other than 409 indicates transport-level failure (auth, malformed request, route not found)") and is not in scope of this row.
- [x] **§7.3.5 — `_urllib_transport` HTTP-error body-parse path** — currently uncovered (live-verifier finding). Test name: `test_urllib_transport_parses_409_body` (`tests/test_lease_plane_retry_and_transport.py`).
...
- [x] **§7.11.3 — `release_reason='forced'` reused for deprecation events; vocabulary unchanged** — Phase 2 sweep events emit with existing `'forced'` value, distinguished by `event_type='lease.deprecation_swept'`. Closes live-verifier DRIFT-1. Test coverage: `test_deprecation_sweep_predicate_idempotent` and `test_deprecation_sweep_emits_lease_deprecation_swept_events` (`tests/test_lease_plane_deprecate_cli.py`) jointly assert that swept leases carry `release_reason='forced'` and emit `event_type='lease.deprecation_swept'` (the `forced_deprecation` rejection from council DRIFT-1 is structurally enforced by the CHECK on `release_reason` — only `'forced'` survives DB validation).
...
- [x] **`models.py AcquireHeldByOther`** — extend with `surface_id`, `blocking_lease_id`, `retry_after_hint_ms` per §7.3.2 (committed in v0.7; not yet in code). Test name: `test_held_by_other_includes_v0_7_extended_fields` (`tests/test_lease_plane_held_by_other_v0_8.py`).
...
- [x] **`agents/sentinel/agent.py`** — add alarm rule keyed on `event_type='forced'` from `lease_plane_events`. Live-verifier confirmed no such rule exists today (Finding 5 SOURCE_ONLY). Per §7.10 + §7.11.5, the rule fires per-event for ad-hoc force-release, batched for deprecation sweeps. Test coverage: `test_sentinel_force_release_alarm_per_event` and `test_sentinel_force_release_alarm_dedupes_via_cursor` (`tests/test_sentinel_forced_release_alarm.py`); rule lives in `agents/sentinel/forced_release_alarm.py`.
