...
class DialecticDBPostgres:
    ...
    def __init__(self, pool=None):
        ...
        Args:
            pool: asyncpg connection pool (from src.db.get_db())
        """
        self._pool = pool
    ...
    async def create_session(
        ...
        async with self._pool.acquire() as conn:
            try:
                # Note: Actual DB schema uses session_id, phase, status, and _json suffix columns
                await conn.execute("""
                ...
                """,
                    ...
                    paused_agent_id,
                    reviewer_agent_id,
                    DialecticPhase.THESIS.value,  # Initial phase
    ...
    async def get_session(self, session_id: str) -> Optional[Dict[str, Any]]:
        """Get session by ID with all messages."""
        async with self._pool.acquire() as conn:
            # Get session (actual schema uses session_id as PK)
            ...
            # Handle _json suffix columns - rename for compatibility
    ...
    async def get_session_by_agent(self, agent_id: str, active_only: bool = True) -> Optional[Dict[str, Any]]:
        """Get session where agent is paused agent or reviewer."""
        async with self._pool.acquire() as conn:
            status_filter = "AND status NOT IN ('resolved', 'timeout', 'abandoned')" if active_only else ""
    ...
    async def get_all_sessions_by_agent(self, agent_id: str) -> List[Dict[str, Any]]:
        ...
        async with self._pool.acquire() as conn:
            rows = await conn.fetch("""
                SELECT session_id FROM core.dialectic_sessions
                WHERE (paused_agent_id = $1 OR reviewer_agent_id = $1)
                AND status NOT IN ('resolved', 'timeout', 'abandoned')
                ORDER BY created_at DESC
            """, agent_id)
    ...
    async def get_active_sessions(self, limit: int = 100) -> List[Dict[str, Any]]:
        ...
        async with self._pool.acquire() as conn:
            rows = await conn.fetch("""
                ...
                WHERE status NOT IN ('resolved', 'timeout', 'abandoned')
                ORDER BY created_at DESC
                LIMIT $1
            """, limit)
            ...
            for row in rows:
                session = dict(row)
                # session_id is already the column name in actual schema
    ...
    async def get_stats(self) -> Dict[str, Any]:
        ...
        async with self._pool.acquire() as conn:
            ...
            # Session counts by session_type
            rows = await conn.fetch("""
...
# =========================================================================
# Async Wrapper Functions (Backend-Agnostic)
# =========================================================================

_db_instance = None  # Can be DialecticDB (SQLite) or DialecticDBPostgres
_db_lock: Optional[asyncio.Lock] = None


async def get_dialectic_db():
    """
    Get singleton dialectic database instance.

    Backend selection:
    - UNITARES_DIALECTIC_BACKEND=postgres -> PostgreSQL
    - UNITARES_DIALECTIC_BACKEND=sqlite   -> SQLite (default)
    - If DB_BACKEND=postgres, defaults to postgres for consistency
    """
    import os
    ...
    async with _db_lock:
        if _db_instance is None:
            backend = os.getenv("UNITARES_DIALECTIC_BACKEND", "").lower()

            # Default to postgres if main DB is postgres
            if not backend and os.getenv("DB_BACKEND", "").lower() == "postgres":
                backend = "postgres"

            if backend == "postgres":
                logger.info("Using PostgreSQL dialectic backend")
                _db_instance = DialecticDBPostgres()
                await _db_instance.init()
            else:
                logger.info("Using SQLite dialectic backend")
                _db_instance = DialecticDB()

        return _db_instance


async def _call_backend(db, method_name: str, *args, **kwargs):
    """
    Call a method on either SQLite (sync) or Postgres (async) backend.

    SQLite methods are synchronous and need run_in_executor.
    Postgres methods are already async and can be awaited directly.
    """
    method = getattr(db, method_name)

    if isinstance(db, DialecticDBPostgres):
        # Postgres backend - methods are already async
        return await method(*args, **kwargs)
    else:
        # SQLite backend - wrap sync methods in executor
...
async def create_session_async(**kwargs) -> Dict[str, Any]:
    ...
    return await _call_backend(db, "create_session", **kwargs)


async def get_session_async(session_id: str) -> Optional[Dict[str, Any]]:
    """Async wrapper for get_session."""
    db = await get_dialectic_db()
    return await _call_backend(db, "get_session", session_id)
...
async def has_recently_reviewed_async(reviewer_id: str, paused_agent_id: str, hours: int = 24) -> bool:
    ...
    db = await get_dialectic_db()
    return await _call_backend(db, "has_recently_reviewed", reviewer_id, paused_agent_id, hours)


async def add_message_async(**kwargs) -> int:
    """Async wrapper for add_message."""
    db = await get_dialectic_db()
    return await _call_backend(db, "add_message", **kwargs)


async def update_session_phase_async(session_id: str, phase: str) -> bool:
    """Async wrapper for update_session_phase."""
    db = await get_dialectic_db()
    return await _call_backend(db, "update_session_phase", session_id, phase)
...
async def update_session_reviewer_async(session_id: str, reviewer_agent_id: str) -> bool:
    ...
    db = await get_dialectic_db()
    return await _call_backend(db, "update_session_reviewer", session_id, reviewer_agent_id)
