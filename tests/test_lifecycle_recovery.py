...
# ============================================================================
# handle_resume_agent — unstick must actually cancel + respawn the underlying
# asyncio Task, not just flip a status flag in Postgres. Regression for the
# 2026-04-14 dogfood finding documenting that the dashboard's "unstick"
# button was theater for system-task agents like eisv-sync-task.
# ============================================================================

class TestResumeAgentUnstickRespawn:
    """Pin the new behavior: handle_resume_agent must call
    cancel_and_respawn_task for known system agents when unstick=True."""
    ...
    @pytest.mark.asyncio
    async def test_unstick_eisv_sync_task_calls_cancel_and_respawn(self, server):
        """When unstick=True is sent for eisv-sync-task, the resume handler
        MUST call cancel_and_respawn_task('eisv_sync') AND include the
        result in the response. Without this, the unstick is theater — the
        hung asyncio Task stays hung."""
        meta = make_agent_meta(status="active")  # system task is permanently active
        server.agent_metadata = {"eisv-sync-task": meta}
        ...
        def fake_respawn(name):
            respawn_calls.append(name)
            return respawn_result

        with patch_lifecycle_server(
            server, require_registered=("eisv-sync-task", None)
        ), patch(
            "src.mcp_handlers.lifecycle.operations.agent_storage"
        ) as mock_storage, patch(
            "src.background_tasks.cancel_and_respawn_task",
            side_effect=fake_respawn,
        ):
            mock_storage.update_agent = AsyncMock()
            ...
            result = await handle_resume_agent({
                ...
                "unstick": True,
                "reason": "dashboard unstick",
            })
            data = _parse(result)

        assert data["success"] is True
        assert respawn_calls == ["eisv_sync"], \
            "cancel_and_respawn_task('eisv_sync') must be called exactly once"
        assert data["task_restart"] == respawn_result

    @pytest.mark.asyncio
    async def test_unstick_unknown_agent_skips_task_restart(self, server):
        """For non-system agents (regular paused Claude sessions), the
        unstick path should still do the flag flip but NOT attempt a task
        restart — there's no asyncio Task to restart on the server side."""
        meta = make_agent_meta(status="paused")
        server.agent_metadata = {"some-claude-session": meta}

        respawn_calls = []

        def fake_respawn(name):
            respawn_calls.append(name)
            return {"restarted": False, "previous_state": "unknown", "reason": "x"}

        with patch_lifecycle_server(
            server, require_registered=("some-claude-session", None)
        ), patch(
            "src.mcp_handlers.lifecycle.operations.agent_storage"
        ) as mock_storage, patch(
            "src.background_tasks.cancel_and_respawn_task",
        ...
        assert respawn_calls == [], \
            "Non-system agents must not trigger cancel_and_respawn_task"
        assert "task_restart" not in data, \
            "task_restart key should be absent when no task was restarted"

    @pytest.mark.asyncio
    async def test_unstick_swallows_respawn_failure(self, server):
        """If cancel_and_respawn_task raises (e.g. background_tasks not
        importable in this test context), the resume must still succeed
        for the operator — the flag flip already happened. The failure
        gets reported in task_restart so it isn't silent."""
        ...
        assert data["success"] is True
        assert data["task_restart"]["restarted"] is False
        assert "boom" in data["task_restart"]["reason"]
