...
# ============================================================================
# System-agent void-pause exemption (eisv-sync-task and similar)
# ============================================================================


class TestSystemAgentVoidExemption:
    """Verify that agents tagged "system" are exempt from void_active pause.

    Context: sensor conduits like eisv-sync-task push sensor-derived EISV
    into governance. A calm Lumen (I >> E) accumulates negative V from
    valid readings, which eventually flips void_active true and triggers
    GovernanceConfig.make_decision's unconditional pause. That pause is
    appropriate for reasoning agents but wrong for sensor conduits. The
    monitor's make_decision wrapper converts void_pause → proceed for
    system-tagged agents and leaves all other pauses intact.
    """

    def _make_tagged_monitor(self, tags):
        """Build a monitor whose agent_id resolves to a metadata record with the given tags."""
        from src.agent_metadata_model import agent_metadata, AgentMetadata

        agent_id = f"test-sys-{id(tags)}"
        ...
        try:
            return UNITARESMonitor(agent_id, load_state=False), agent_id
        finally:
            pass  # metadata cleanup handled in each test via teardown

    def _cleanup(self, agent_id):
        from src.agent_metadata_model import agent_metadata
        agent_metadata.pop(agent_id, None)

    def test_void_pause_suppressed_for_system_agent(self, monkeypatch):
        """A system-tagged agent with void_active=True should proceed, not pause."""
        mon, agent_id = self._make_tagged_monitor(["system"])
        try:
            # Force void_active to simulate E-I imbalance
            mon.state.void_active = True
            mon.state.unitaires_state.V = -0.25

            decision = mon.make_decision(risk_score=0.2, unitares_verdict="safe")

            assert decision["action"] == "proceed"
            assert decision["sub_action"] == "system_exempt"
            assert "suppressed_decision" in decision
            assert decision["suppressed_decision"]["sub_action"] == "void_pause"
        finally:
            self._cleanup(agent_id)

    def test_void_pause_still_fires_for_regular_agent(self):
        """A regular (untagged) agent with void_active=True should still pause."""
        mon, agent_id = self._make_tagged_monitor([])  # no tags
        try:
            mon.state.void_active = True
            mon.state.unitaires_state.V = -0.25
            ...
            assert "suppressed_decision" not in decision
        finally:
            self._cleanup(agent_id)

    def test_void_pause_still_fires_for_pioneer_only_agent(self):
        """Pioneer tag alone does not exempt from void pause — the tag is "system"."""
        mon, agent_id = self._make_tagged_monitor(["pioneer"])
        try:
            mon.state.void_active = True
            mon.state.unitaires_state.V = -0.25
    ...
    def test_system_agent_still_pauses_on_coherence_critical(self):
        """Exemption is narrow: coherence_pause is not suppressed."""
        mon, agent_id = self._make_tagged_monitor(["system", "pioneer"])
        try:
            mon.state.void_active = False
            mon.state.coherence = 0.0  # well below critical threshold
            mon.state.coherence_history = [0.0, 0.0, 0.0]

            decision = mon.make_decision(risk_score=0.2, unitares_verdict="safe")

            # With coherence at 0, either coherence_pause or a caution/guide
            # path fires — the important thing is that we do NOT return
            # a system_exempt bypass here, because the exemption is scoped
            # to void_pause only.
            assert decision.get("sub_action") != "system_exempt"
        finally:
            self._cleanup(agent_id)

    def test_system_agent_unchanged_when_void_inactive(self):
        """With void_active=False, the system agent takes the normal proceed path."""
        mon, agent_id = self._make_tagged_monitor(["system"])
        try:
            mon.state.void_active = False
            mon.state.unitaires_state.V = 0.05  # small, well under threshold
            mon.state.unitaires_state.E = 0.7
            mon.state.unitaires_state.I = 0.7

            decision = mon.make_decision(risk_score=0.2, unitares_verdict="safe")

            assert decision["action"] == "proceed"
            # Should NOT be the exemption path — should be the regular approve
            assert decision.get("sub_action") != "system_exempt"
        finally:
            self._cleanup(agent_id)

    def test_is_system_agent_true_for_tagged(self):
        mon, agent_id = self._make_tagged_monitor(["system"])
        try:
            assert mon._is_system_agent() is True
        finally:
            self._cleanup(agent_id)

    def test_is_system_agent_false_for_missing_metadata(self):
        mon = UNITARESMonitor("nonexistent-agent-xyz", load_state=False)
        assert mon._is_system_agent() is False

    def test_is_system_agent_false_for_untagged(self):
        mon, agent_id = self._make_tagged_monitor([])
        try:
            assert mon._is_system_agent() is False
        finally:
            self._cleanup(agent_id)
